...
def get_data(filters):
	...
	depreciation_amount_map = get_depreciation_amount(filters,finance_book)
...
def get_depreciation_amount(filters,finance_book):
	records = {}
	filters_for_query = {"docstatus": 1}

	if filters.only_existing_assets:
		filters_for_query["is_existing_asset"] = 1

	if filters.asset_category:
		filters_for_query["asset_category"] = filters.asset_category

	if filters.cost_center:
		filters_for_query["cost_center"] = filters.cost_center

	if filters.status:
		if filters.status == "In Location":
			filters_for_query["status"] = ["not in", ["Sold", "Scrapped", "Capitalized", "Decapitalized"]]
		else:
			filters_for_query["status"] = ["in", ["Sold", "Scrapped", "Capitalized", "Decapitalized"]]
	start_date = (
		filters.from_date if filters.filter_based_on == "Date Range" else filters.year_start_date
	)
	end_date = filters.to_date if filters.filter_based_on == "Date Range" else filters.year_end_date
	assets = frappe.get_all("Asset", filters=filters_for_query)
	for asset in assets:
		filters_for_child = {}
		if filters.filter_based_on in ("Date Range", "Fiscal Year"):
			filters_for_child['schedule_date'] = ["between", [start_date, end_date]]
		if finance_book:
			filters_for_child['finance_book'] = finance_book
		filters_for_child['parent'] = asset.name
		depreciation_docs = frappe.get_all("Depreciation Schedule",filters_for_child,'depreciation_amount')
		depreciation_amount = 0
		for depreciation_doc in depreciation_docs:
			depreciation_amount += depreciation_doc.depreciation_amount
		records.update({asset.name:round(depreciation_amount,2)})
