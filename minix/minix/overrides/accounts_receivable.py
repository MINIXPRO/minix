...
class ReceivablePayableReport(object):
	...
	def set_ageing(self, row):
		...
		if getdate(entry_date) > getdate(self.filters.report_date):
			row.range1 = row.range2 = row.range3 = row.range4 = row.range5 = row.range6 = row.range7 = row.range8 = row.range9 = row.range10 = 0.0

		row.total_due = row.range1 + row.range2 + row.range3 + row.range4 + row.range5 + row.range6 + row.range7 + row.range8 + row.range9 + row.range10
	...
	def get_ageing_data(self, entry_date, row):
		...
		if not (
			...
			and self.filters.range5 and self.filters.range6 and self.filters.range7 and self.filters.range8
			and self.filters.range9 and self.filters.range10
		):
			self.filters.range1, self.filters.range2, self.filters.range3, self.filters.range4, self.filters.range5, self.filters.range6, self.filters.range7, self.filters.range8, self.filters.range9, self.filters.range10 = (
				30,
				60,
				90,
				120,
	...
	def setup_ageing_columns(self):
		...
		for i, label in enumerate(
			[
				...
				"{range3}-{range4}".format(
					range3=cint(self.filters["range4"]) + 1, range4=self.filters["range5"]
				),
				"{range3}-{range4}".format(
					range3=cint(self.filters["range5"]) + 1, range4=self.filters["range6"]
				),
				"{range3}-{range4}".format(
					range3=cint(self.filters["range6"]) + 1, range4=self.filters["range7"]
				),
				"{range3}-{range4}".format(
					range3=cint(self.filters["range7"]) + 1, range4=self.filters["range8"]
				),
				"{range3}-{range4}".format(
					range3=cint(self.filters["range8"]) + 1, range4=self.filters["range9"]
				),
				"{range3}-{range4}".format(
					range3=cint(self.filters["range9"]) + 1, range4=self.filters["range10"]
				),
				_("{range4}-Above").format(range4=cint(self.filters["range10"]) + 1),
			]
		):
	...
	def get_chart_data(self):
		...
		for row in self.data:
			...
			if not cint(row.bold):
				values = [row.range1, row.range2, row.range3, row.range4, row.range5, row.range6, row.range7, row.range8, row.range9, row.range10]
